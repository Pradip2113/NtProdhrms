...
class EmployeeCheckin(Document):	
		...
		def calot(self):
			...
			doc=frappe.db.get_list("Employee Checkin",fields=["time","employee","log_type"])
			...
			if self.log_type=="IN":
				...
				for d in doc:
					...
					if(self.employee==d.employee and dt.strptime(str(date2), "%Y-%m-%d")==dt.strptime(str(date1), "%Y-%m-%d")):
						if d.log_type=="IN":
							
							res=(dt.strptime(str(self.time), "%Y-%m-%d %H:%M:%S") - dt.strptime(str(d.time), "%Y-%m-%d %H:%M:%S"))
							break
       						
		
				if str(res)[0]!='-':		
					if(dt.strptime(str(res), "%H:%M:%S")<=dt.strptime(str(workinghours), "%H:%M:%S")):
						self.overtime='0'
